...
while line != "":
    if "Country" in line:
        ...
        value = int(temp[9:])
        if value == 0:
            output.append([line[9:11], 0])
        else:
            output.append([{"v": line[9:11], "f": codeToCountry[line[9:11].lower()]}, {"v": math.log(int(temp[9:]), 10), "f":int(temp[9:])}])
        line = f.readline()

print(output)
q.write(str(output).replace("'v'", "v").replace("'f'", "f"))
